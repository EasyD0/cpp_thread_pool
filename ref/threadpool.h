...
class ThreadPool {
   private:
    ...
    std::queue<Task_type> task_Q;  // 任务队列
    std::atomic_int task_num;      // 任务的数量
    unsigned task_num_MAX;         // 任务队列数量上限阈值

    std::mutex task_mtx;  // 保证任务队列的线程安全
   ...
   public:
    ...
    auto submitTask(Func&& func,
        ...
        // 将任务函数变成一个指针
        auto task_uptr = std::make_shared<std::packaged_task<RType()>>(
        ...
        task_Q.emplace([task_uptr]() { (*task_uptr)(); });
        task_num++;
        ...
        if (poolMode_ == PoolMode::MODE_CACHED && task_num > idleThreadSize_ &&
            curThreadSize_ < thread_num_MAX) {
                    ...
                    if (poolMode_ == PoolMode::MODE_CACHED) {
                        ...
                        if (std::cv_status::timeout ==
                            ...
                            if (dur.count() >= THREAD_MAX_IDLE_TIME &&
                                curThreadSize_ > default_thread_num) {
                                // 开始回收当前线程
                                // 记录线程数量的相关变量的值修改
